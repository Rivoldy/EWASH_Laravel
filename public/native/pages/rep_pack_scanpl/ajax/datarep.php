...
?>
  <table class="table table-sm table-hover table-bordered table-striped table-bordered" style="width: 100%;" id="tabsku">
    ...
    <tbody>
      <?php
      $q = mysqli_query($conn, "SELECT *,sum(nw) as nett, sum(gw) as gross, count(distinct(bid)) as karung from t_scale where kp='$kp' $ftgl group by color");
      ...
      ?>
        <tr>
          ...
          $jmlpcs = 0;
          for ($i = 0; $i < count($nmsize); $i++) {
            $qsi = mysqli_query($conn, "SELECT sum(qty) as qty from t_scale where kp='$kp' and color='$color' and size='$nmsize[$i]'");
          ...
          <td class="text-right"><?= number_format($dt['nett'], 2) ?></td>
          <td class="text-right"><?= number_format($dt['gross'], 2) ?></td>
        </tr>
      <?php
    ...
    <tfoot>
      <tr>
        ...
        <th class="text-right"><?= number_format($totnw, 2) ?></th>
        <th class="text-right"><?= number_format($totgw, 2) ?></th>
      </tr>
    </tfoot>
  </table>
  <script>
    ...
    $('div.toolbar').html('<a target="_blank" href="http://127.0.0.1:8000/storage/ewash/pages/rep_pack_scanpl/report.php?kp=<?= $kp . '&' . $utgl ?>" class="btn btn-danger"><i class="fal fa-file-pdf"></i> Export</a>');
  </script>
  <style>
    .toolbar {
      float: right;
