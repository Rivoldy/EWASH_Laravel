...
<?php } else if ($tipe == "pl") {
    ...
    $querycount = mysqli_query($conn, "SELECT * FROM t_bag where pl_id='$pl_id' GROUP BY bid");
    $totalFiltered = $totalData = mysqli_num_rows($querycount);
    $query = mysqli_query($conn, "SELECT * FROM t_bag where pl_id='$pl_id' GROUP BY bid order by color");
    $data = array();
    if (!empty($query)) {
        while ($r = $query->fetch_array()) {
            $q1 = mysqli_query($conn, "SELECT SUM(qty) AS total FROM t_bag WHERE pl_id='$pl_id' AND bid = '{$r['bid']}'");
            $r1 = mysqli_fetch_array($q1);
