...
?>
  <table class="table table-sm table-hover table-bordered table-striped table-bordered" style="width: 100%;" id="tabsku">
    ...
    <tbody>
      <?php
      $q = mysqli_query($conn, "SELECT * from t_scale where kp='$kp' $ftgl order by bid");
  ...
  <script>
    ...
    $('div.toolbar').html('<a target="_blank" href="http://127.0.0.1:8000/storage/ewash/pages/rep_pack_detailpl/report.php?kp=<?= $kp . '&' . $utgl ?>" class="btn btn-danger"><i class="fal fa-file-pdf"></i> Export</a>');
  </script>
  <style>
    .toolbar {
      float: right;
