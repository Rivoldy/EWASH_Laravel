...
<?php } else if ($tipe == "pl") {
    ...
    $querycount = mysqli_query($conn, "SELECT * FROM t_scale where pl_id='$pl_id' limit 10");
    $totalFiltered = $totalData = mysqli_num_rows($querycount);
    $query = mysqli_query($conn, "SELECT *, SUM(qty) AS total FROM t_scale where pl_id='$pl_id' GROUP BY bid order by loading desc limit 10");
    $data = array();
    if (!empty($query)) {
    ...
    $style = $_POST['style'];
    $kp = $_POST['kp'];
    $q = mysqli_query($conn, "SELECT * FROM t_scale WHERE bid = '$bid' AND style = '$style' AND kp = '$kp'");
    if (mysqli_num_rows($q) == 0) {
        array_push($result, array(
            'notif' => 'not',
        ));
