    ...
    <table id="example" class="table table-sm table-hover table-striped table-bordered" style="width:100%">
        ...
        <tbody>
            ...
            while ($r = mysqli_fetch_assoc($q)) {
                $q1 = mysqli_query($conn, "SELECT SUM(qty) AS qty FROM t_scale WHERE pl_id = '{$r['id']}'");
                ...
                $bag = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM t_scale WHERE pl_id = '{$r['id']}' GROUP BY bid"));
            ...
            ?>
                <tr>
                    ...
                    <td align="center">
                        ...
                        <button class="btn btn-success" <?= $ex ?> onclick="expo('<?= $r['id'] ?>')" title="Export"><i class="fas fa-file-excel"></i></button>
                    </td>
                <?php } ?>
        </tbody>
    </table>
...
<?php } else if ($tipe == "pl") {
    ...
    echo '<option></option>';
    $q = mysqli_query($conn, "SELECT * FROM t_scale WHERE style='$style' GROUP BY kp");
    ...
    $style = array();
    $q1 = mysqli_query($conn, "SELECT * FROM t_scale GROUP BY style");
    ...
    $kp = array();
    $q1 = mysqli_query($conn, "SELECT * FROM t_scale WHERE style = '{$r['style']}' GROUP BY kp");
    while ($r1 = mysqli_fetch_array($q1)) {
        array_push($kp, array(
            'kp' => $r1['kp'],
        ));
    ...
    $id = $_POST['id'];
    $s = mysqli_query($conn, "SELECT * FROM t_scale WHERE pl_id = '$id'");
    if (mysqli_num_rows($s) == 0) {
    ...
    $querycount = mysqli_query($conn, "SELECT * FROM t_scale where pl_id='$pl_id' GROUP BY bid");
    $totalFiltered = $totalData = mysqli_num_rows($querycount);
    $query = mysqli_query($conn, "SELECT * FROM t_scale where pl_id='$pl_id' GROUP BY bid order by color");
    $data = array();
    if (!empty($query)) {
        while ($r = $query->fetch_array()) {
            $q1 = mysqli_query($conn, "SELECT SUM(qty) AS total FROM t_scale WHERE pl_id='$pl_id' AND bid = '{$r['bid']}'");
            $r1 = mysqli_fetch_array($q1);
            $nestedData['bid'] = $r['bid'];
            $nestedData['style'] = $r['style'];
            $nestedData['kp'] = $r['kp'];
            $nestedData['qty'] = $r1['total'];
    ...
    $style = $_POST['style'];
    $kp = $_POST['kp'];
    $q = mysqli_query($conn, "SELECT * FROM t_scale WHERE bid = '$bid' AND kp = '$kp'");
    if (mysqli_num_rows($q) == 0) {
        array_push($result, array(
            'notif' => 'not',
        ));
        ...
        if ($d['pl_id'] != null) {
            ...
            $q = mysqli_query($conn, "UPDATE t_scale SET pl_id = '$pl_id', loading = NOW(), pic_loading = '$nik' WHERE bid = '$bid'");
            $q2 = mysqli_query($conn, "SELECT SUM(qty) AS qty FROM t_scale WHERE pl_id = '$pl_id'");
    ...
    $id = $_POST['id'];
    $q = mysqli_query($conn, "UPDATE t_scale SET closing_status = '1', closing = NOW() WHERE pl_id = '$id'");
    ...
    $id = $_POST['id'];
    $value = $_POST['value'];
    $q = mysqli_query($conn, "UPDATE t_scale SET closing_status = null, closing = null WHERE pl_id = '$id'");
